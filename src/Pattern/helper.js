...
export function isGeneralValid(pattern) {
  return (
    pattern !== undefined &&
    isPositive(pattern.length) &&
    isPositive(pattern.width) &&
    isPositive(pattern.height)
  );
}
...
export function usePatternMeasurement(pattern) {
  ...
  let l = parseFloat(pattern.length);
  let w = parseFloat(pattern.width);
  let h = parseFloat(pattern.height);
